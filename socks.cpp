...
*                                                                                                        *
\********************************************************************************************************/
void CSocks::SetSocksAddress( const char *in_szSocksAddress )
{
	strcpy( m_SocksAddress, in_szSocksAddress );
}
/********************************************************************************************************\
...
int CSocks::Connect()
{
	...
	try
	{
		...
		int nNetTimeout = 25000;  //超时时间，因为设置的超时时间大大高于心跳，所以理论上不可能出现接收数据的时间大于超时时间，所以肯定是套接字发生了问题，例如拔出网线
		...
		else if ( m_Version == SOCKS_VER5 )
		{
			ret_val = ConnectToSocksFive( m_Socket, 
								...
								sizeof(struct sockaddr),
								(struct sockaddr*)&Socks_Addr,
								sizeof(struct sockaddr),0, m_DesitnationAddress, htons( m_DestinationPort ) );
			if ( ret_val != 0 ) 
			{
					//Failed to Establish connection through Socks
					return ret_val;
						...
						short destPort )
{
		...
		try
		{
			...
			char packet[4] = {0};				//Allocate a packet
    		...
    		//Send the packet
			long bytesSend = 0;
    		bytesSend =	send( in_Socket, packet, 4, 0 );
			...
			{				
				...
				packet1[4] = strlen(destAddress); 
				memcpy(packet1+5, destAddress, packet1[4]);

				memcpy( packet1+packet1[4]+5,							//Copy the port
					(char *)(&destPort),
					2 );
							
    			//Send the packet
				bytesSend = 0;
				//bytesSend = send( in_Socket, packet1, 10, 0 );
    			bytesSend = send( in_Socket, packet1, packet1[4]+7, 0 );
